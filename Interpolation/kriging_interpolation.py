...
# -*- coding: utf-8 -*-
"""
Created on Tue Aug 17 10:19:27 2021
...
with open('interp.csv', 'a') as f:
    ...
    for dt in range(0, len(d[0])):
        # if int(t[loc][dt].split(':')[0]) >= 12 and int(t[loc][dt].split(':')[0]) <= 17:
            ...
            kriging_o3 = []
            for loc in range(0, len(names)):
                kriging_o3.append(float(o3[loc][dt]))
            ...
            mesh_lat = np.transpose(mesh_lat)
            for i in range(0, len(z)):
                for j in range(0, len(z)):
                    f.write(d[loc][dt] + ',' + t[loc][dt] + ',' + str(mesh_lat[i][j]) + ',' + str(mesh_lon[i][j]) + ',' + str(z[i][j]))
                    f.write('\n')
            ...
            for add_loc in range(0, len(additional_sites)):
                ...
                interp_add_o3_data[add_loc].append(z[y[add_loc]][x[add_loc]])
            ...
            for loc in range(0, len(names)):
                ...
                # get interpolated data
                interp_o3_data[loc].append(z[y[loc]][x[loc]])
                interp_d_data[loc].append(d[loc][dt])
                interp_t_data[loc].append(t[loc][dt])
                interp_o3.append(z[y[loc]][x[loc]])
            ...
            fig, ax = plt.subplots()
            ax.imshow(z, cmap=plt.cm.Reds, interpolation='none', extent=[min(lon),max(lon),min(lat),max(lat)])
            mesh = ax.pcolormesh(lon,lat,z,shading='auto')
            cbar = fig.colorbar(mesh)
            cbar.mappable.set_clim(mini,maxi)
            cbar.remove()
            
            # scatter plot for 15 ML sites
            plt.scatter(longitude, latitude, 50, c=kriging_o3)
            plt.xlim([min(lon), max(lon)])
            plt.ylim([min(lat), max(lat)])
            cbar=plt.colorbar()
            cbar.mappable.set_clim(mini,maxi)
            # cbar.remove()
            plt.title('Predicted Ozone on ML Heatmap')
...
# scatter plots for interp O3 from Kriging and observed O3      
...
# scatter plots for interp additional O3 from Kriging and observed O3 from addtionial sites
print('Plotting scatter plots for additional O3 from Kriging and observed O3 from addtionial sites ...')
...
for i in range(0, len(additional_sites)):
    ...
    plt.close()
print('Plotting scatter plots for additional O3 from Kriging and observed O3 from addtionial sites: completed')
...
print('Kriging Interplation Successfully')