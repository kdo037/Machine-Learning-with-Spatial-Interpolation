...
# -*- coding: utf-8 -*-
"""
Created on Tue Aug 24 11:45:47 2021
...
for loc in range(0, len(names)):
    lines = []
    with open('/home/khanh/hdd1/ML_2020_paper/ML_results/' + wroteName[loc] + '.csv', 'r', encoding = 'utf-8', errors = 'ignore') as readFile:
...
for loc in range(0, len(additional_sites)):
    lines = []
    with open('/home/khanh/hdd1/ML_2020_paper/aqmis/aq/ozone_additional_sites/' + additional_sites[loc] + '.csv', 'r', encoding = 'utf-8', errors = 'ignore') as readFile:
        reader = csv.reader(readFile)
        lines = list(reader)
    d = []
    t = []  
...
with open('interp.csv', 'a') as f:
    ...
    for dt in range(0, len(d[0])):
        # if int(t[loc][dt].split(':')[0]) >= 12 and int(t[loc][dt].split(':')[0]) <= 17:
            ...
            # plot shapefile
            shapefile = gpd.read_file('/home/khanh/hdd1/CMAQ_2020/evaluations/spatial_plot/cb_2018_us_county_20m.shp')
...
# scatter plots for interp O3 from cubic and observed O3      
...
# scatter plots for interp additional O3 from cubic and observed O3 from addtionial sites
print('Plotting scatter plots for additional O3 from cubic and observed O3 from addtionial sites ...')
...
for i in range(0, len(additional_sites)):
    ...
    plt.close()
print('Plotting scatter plots for additional O3 from cubic and observed O3 from addtionial sites: completed')
...
print('cubic Interplation Successfully')
